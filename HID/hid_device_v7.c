...
static void uart_rx_task(void *arg)
{
    ...
    // Track last states for hold support
            ...
            if (frame_len >= 3) {
                ...
                if (frame_len == expected_len) {
                    ...
                    if (validate_checksum(frame_buf, expected_len)) {
                        ...
                        if (tud_hid_ready()) {
                            bool changed = (modifier != last_modifier) || memcmp(keycode_array, last_keys, 6) != 0;
                            if (changed) {
                                tud_hid_keyboard_report(0, modifier, keycode_array);
                                last_modifier = modifier;
                                memcpy(last_keys, keycode_array, 6);
                            }
                        }
                        ...
                        ESP_LOGI("HID_RX", "Keyboard: mod=0x%02X keys=%c %c %c %c %c %c",
                            ...
                            if (tud_hid_ready()) {
                                ...
                                // only send button change when state changes
                                if (buttons != last_buttons) {
                                    tud_hid_mouse_report(1, buttons, 0, 0, 0, 0);
