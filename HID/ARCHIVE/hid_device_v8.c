...
static volatile bool link_obstructed = true;   
static volatile uint32_t last_good_ms = 0;
static volatile uint32_t last_any_ms  = 0;
static volatile uint16_t recent_frame_errs = 0;

#define OBSTRUCT_MS        200   // no valid frame for 200 ms -> obstructed
#define ERR_DECAY_MS       500   // decay window for FE count
#define FE_TRIGGER_COUNT   3     // >=3 framing errors in window -> obstructed
...
// Monitors UART event queue (framing/parity errors) and timeouts
static void link_monitor_task(void *arg) {
    uart_event_t ev;
    uint32_t last_decay = now_ms();
    ...
    for (;;) {
        while (xQueueReceive(uart_queue, &ev, pdMS_TO_TICKS(10))) {
            last_any_ms = now_ms();
            if (ev.type == UART_FRAME_ERR || ev.type == UART_PARITY_ERR) {
                recent_frame_errs++;
            }
        }
        uint32_t t = now_ms();
        if (t - last_decay >= ERR_DECAY_MS) {
            if (recent_frame_errs) recent_frame_errs--;
            last_decay = t;
        }
        bool by_timeout = (t - last_good_ms) > OBSTRUCT_MS;
        bool by_errors  = (recent_frame_errs >= FE_TRIGGER_COUNT);
        bool new_state  = (by_timeout || by_errors);

        if (new_state != link_obstructed) {
            link_obstructed = new_state;
            ESP_LOGW("LINK","State = %s (idle=%ums, FE_recent=%u)",
                     link_obstructed ? "OBSTRUCTED" : "CLEAR",
                     (t - last_good_ms), recent_frame_errs);
        }
...
void app_main(void)
{
    ...
    xTaskCreate(link_monitor_task, "link_monitor", 2048, NULL, 6, NULL);
