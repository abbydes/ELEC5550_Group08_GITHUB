...
#define UART_RX_PIN        17              // Match sender TX
...
static bool validate_checksum(uint8_t *frame, int len) {
    ...
    uint8_t sum = 0;
    for (int i = 0; i < len - 1; i++) {   // exclude last byte (checksum)
        sum += frame[i];
    }

    uint8_t expected = frame[len - 1];
    if (sum == expected) {
        ESP_LOGI("UART_RX", "Checksum OK (0x%02X)", expected);
        return true;
    } else {
        ESP_LOGW("UART_RX", "Checksum FAIL: got 0x%02X, expected 0x%02X", expected, sum);
        return false;
    }
}
...
// <--- NEW TASK for UART deserialisation
static void uart_rx_task(void *arg)
{
    ...
    while (1) {
        ...
        if (len > 0) {
            ...
            // Example: [A5] [4B] [08] [mod] [00] [key] ... [checksum]
            if (data[0] == 0xA5) {
                if (validate_checksum(data, len)) {
                    if (data[1] == 'K') {  // Keyboard frame
                        ...
                        kb_report.keycode[0] = keycode;

                        tud_hid_keyboard_report(0, kb_report.modifier, kb_report.keycode);
                    }
                    else if (data[1] == 'M') { // Mouse frame
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(uart_set_pin(UART_PORT_NUM, UART_RX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
