...
// Helper: convert 8-bit value to binary string
static void byte_to_binary(uint8_t val, char *out)
{
    for (int i = 7; i >= 0; i--) {
        out[7 - i] = (val & (1 << i)) ? '1' : '0';
    }
...
static void key_event_callback(key_event_t *key_event)
{
    ...
    if (KEY_STATE_PRESSED == key_event->state) {
        if (hid_keyboard_get_char(key_event->modifier,
            ...
            char bin_str[9];
            byte_to_binary(key_char, bin_str);

            // Print character with ASCII representations
            printf("Char: %c  ASCII(dec): %d  ASCII(hex): 0x%02X  ASCII(bin): %s\n",
                   key_char, (int)key_char, (unsigned char)key_char, bin_str);
