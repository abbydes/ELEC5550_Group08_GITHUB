                ...
                if (frame_len == expected_len) {
                    ...
                    if (validate_checksum(frame_buf, expected_len)) {
                        ...
                        if (tud_hid_ready()) {
                            ...
                            // --- add this release event ---
                            vTaskDelay(pdMS_TO_TICKS(20)); // small delay so host registers the keypress
                        ...
                        ESP_LOGI("HID_RX", "Keyboard: mod=0x%02X keys=%c %c %c %c %c %c",
                            ...
                            if (tud_hid_ready()) {
                                ...
                                // if it was a click, release buttons after a short delay
                                if (buttons != 0) {
                                    vTaskDelay(pdMS_TO_TICKS(20));
                                    tud_hid_mouse_report(1, 0, 0, 0, 0, 0);
                                }
                            }
                        }
