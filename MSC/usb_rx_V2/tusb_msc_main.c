...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Starting laser UART listener...");
    init_laser_uart();
...
#define LASER_RX_PIN     17   // change to your wiring
#define LASER_TX_PIN     16   // change if needed
...
static void laser_uart_task(void *arg)
{
    ...
    uint8_t buf[LASER_BUF_SIZE];
    int len;
    TickType_t last_rx = xTaskGetTickCount();

    while (1) {
        len = uart_read_bytes(LASER_UART_NUM, buf, sizeof(buf), 500 / portTICK_PERIOD_MS);
        ...
        if (len > 0) {
            ...
            if (!receiving) {
                if (strncmp(p, "FILE:", 5) == 0) {
                    sscanf(p, "FILE:%63s", filename);
                    char fullpath[128];
                    snprintf(fullpath, sizeof(fullpath), "/data/%s", filename);
                    fp = fopen(fullpath, "w");
                    if (!fp) {
                        ESP_LOGE("RX", "Failed to open file %s", fullpath);
                        continue;
                    }
                    receiving = true;
                    ESP_LOGI("RX", "Receiving file %s", fullpath);
                }
            } else {
                if (strncmp(p, "CHUNK", 5) == 0) {
                    int chunk_id, chunk_len;
                    if (sscanf(p, "CHUNK %d %d", &chunk_id, &chunk_len) == 2) {
                        char *payload = strchr(p, '\n');
                        if (payload) {
                            payload++;
                            int data_len = len - (payload - (char*)buf);
                            fwrite(payload, 1, data_len, fp);

                            char ackmsg[32];
                            snprintf(ackmsg, sizeof(ackmsg), "ACK CHUNK %d\n", chunk_id);
                            uart_write_bytes(LASER_UART_NUM, ackmsg, strlen(ackmsg));
                            ESP_LOGI("RX", "Acked chunk %d", chunk_id);
                        }
                    }
                    ...
                    fp = NULL;
                    receiving = false;
                    uart_write_bytes(LASER_UART_NUM, "ACK END\n", 8);
                    ESP_LOGI("RX", "File %s received completely", filename);
                }
            }
        } else {
            // Timeout waiting for data
            if (xTaskGetTickCount() - last_rx > pdMS_TO_TICKS(2000)) {
                if (receiving) {
                    ESP_LOGW("RX", "Timeout mid-transfer, NACK + re-request");
                    if (fp) { fclose(fp); fp = NULL; }
                    receiving = false;
                    uart_write_bytes(LASER_UART_NUM, "NACK\n", 5);
                } else {
                    ESP_LOGW("RX", "No response, re-requesting dummy.txt");
                }
                uart_write_bytes(LASER_UART_NUM, "READ dummy.txt\n", 15);
                last_rx = xTaskGetTickCount(); // reset timer
            }
        }
    }
}
...
static void init_laser_uart(void)
{
    ...
    uart_driver_install(LASER_UART_NUM, LASER_BUF_SIZE * 2, 0, 0, NULL, 0);
    uart_param_config(LASER_UART_NUM, &uart_config);
    uart_set_pin(LASER_UART_NUM, LASER_TX_PIN, LASER_RX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
