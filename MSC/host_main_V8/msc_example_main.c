...
static void retrigger_handshake(void) {
    ...
    if (handshake_task_handle == NULL) {
        ...
        // Just signal the running handshake task to restart
        handshake_triggered = true;
    }
}
...
static void uart_rx_task(void *arg) {
    ...
    while (1) {
        // --- Read and log every incoming byte ---
        int len = uart_read_bytes(UART_PORT_NUM, &byte, 1, pdMS_TO_TICKS(1));
        if (len > 0) {
            // Collect ASCII text for handshake detection
        ...
        // --- FRAME READING SECTION (unchanged) ---
        // Wait for start byte
        uint8_t start = 0;
        while (start != 0xA5) {
            ...
            // Detect if ASCII handshake starts mid-frame
            if (start >= 0x20 && start <= 0x7E) {
                // Collect potential ASCII string
                text_buf[0] = start;
