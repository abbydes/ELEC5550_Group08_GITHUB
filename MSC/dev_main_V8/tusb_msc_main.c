...
int32_t tud_msc_read10_cb(uint8_t lun, uint32_t lba, uint32_t offset, void* buffer, uint32_t bufsize) {
    ...
    ESP_LOGI(TAG, "Sector head:");
    ESP_LOG_BUFFER_HEXDUMP(TAG, &rx_frame[8], 16, ESP_LOG_INFO);
    ESP_LOG_BUFFER_HEXDUMP(TAG, &rx_frame[8 + data_len - 16], 16, ESP_LOG_INFO);

    ESP_LOGI(TAG, "READ10 successful (LBA=%lu, bytes=%u)", lba, data_len);
    return data_len;

    /*if (len == expected_len && rx_frame[0] == 0xA5 && rx_frame[1] == 'D') {
        uint32_t sum = rx_frame[1] + rx_frame[2] + rx_frame[3];
        for (int i = 0; i < payload_len; i++) sum += rx_frame[4 + i];
        ...
        if (rx_frame[len - 1] == checksum) {
            ESP_LOGI(TAG, "READ10 successful (LBA = %lu)", lba);
            memcpy(buffer, &rx_frame[8], bufsize);
            return bufsize;
        } else {;
            ESP_LOGW(TAG, "Checksum mismatch in READ10");
        }
