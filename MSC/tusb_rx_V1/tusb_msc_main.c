...
#define LASER_RX_PIN     16   // change to your wiring
#define LASER_TX_PIN     17   // change if needed
...
static void laser_uart_task(void *arg)
{
    ...
    while (1) {
        int len = uart_read_bytes(LASER_UART_NUM, data, LASER_BUF_SIZE - 1,
                                  100 / portTICK_PERIOD_MS);
        if (len > 0) {
            data[len] = 0; // null terminate

            // Parse "FILE:..." and "CONTENT:..."
            char *filetag = strstr((char*)data, "FILE:");
            char *content = strstr((char*)data, "CONTENT:");
            if (filetag && content) {
                char filename[64];
                sscanf(filetag, "FILE:%63s", filename);

                char *endtag = strstr((char*)data, "END");
                if (endtag) {
                    *endtag = '\0'; // terminate before END
                }

                char *filecontent = content + strlen("CONTENT:");

                // Always write into /data (MSC mount point)
                char fullpath[128];
                ...
                if (fp) {
                    ...
                    fclose(fp);
                    ESP_LOGI("LASER", "Wrote file %s via laser link", fullpath);

                    // If correctly sent, send ACK back
                    const char *ack = "ACK\n";
                    uart_write_bytes(LASER_UART_NUM, ack, strlen(ack));
                } else {
                    ESP_LOGE("LASER", "Failed to open file %s", fullpath);

                    // If incorrectly sent, send NAK back
                    const char *nak = "NAK\n";
                    uart_write_bytes(LASER_UART_NUM, nak, strlen(nak));
                }
            }
        }
    }
