...
static bool validate_checksum(const uint8_t *frame, int len) {
    ...
    uint8_t payload_len = ((uint16_t)frame[2] << 8) | frame[3];
    if (len != payload_len + 4 + 1) return false;

    uint32_t sum = type + frame[2] + frame[3];
...
static void uart_rx_task(void *arg) {
    ...
    while (1) {
        int len = uart_read_bytes(UART_PORT_NUM, rx_frame, sizeof(rx_frame), pdMS_TO_TICKS(200));
        if (len <= 0) continue;
        ...
        uint8_t payload_len = (rx_frame[2] << 8) | rx_frame[3];
        ...
        if (type == 'R') {
            ...
            tx_frame[0] = 0xA5;
            tx_frame[1] = 'D';
            tx_frame[2] = (resp_payload_len >> 8) & 0xFF;
            tx_frame[3] = (resp_payload_len & 0xFF);
            ...
            vTaskDelay(pdMS_TO_TICKS(10)); //5ms pause to allow for Board A to start reading
